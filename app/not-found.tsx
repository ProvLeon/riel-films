...
### Why This Approach Will Help:

1. **No Shared Components**: This version avoids using your shared components like `PageTransition` and `SectionReveal` which might be causing the issues.

2. **Custom Implementations**: Instead, it uses simple custom implementations that provide similar functionality without relying on any client-side hooks.

3. **Isolated Logic**: The 404 page's functionality is completely self-contained, reducing the chance of unexpected interactions with other parts of your application.

4. **Proper Suspense Boundary**: We still maintain the proper Suspense boundary structure for client-side components.

### Alternative Approach: Simplify the 404 Page

If the above solution doesn't work, you might want to try a very simple 404 page that doesn't use any animations or complex components:
...
const SimpleNotFoundContent = () => {
  return (
    <div className="min-h-screen flex items-center justify-center bg-white dark:bg-film-black-950 px-4">
      <div className="max-w-lg w-full text-center">
        <div className="text-8xl font-bold text-film-red-600 dark:text-film-red-500 mb-6">404</div>
        <h1 className="text-3xl font-bold mb-4 text-film-black-900 dark:text-white">Page Not Found</h1>
        <p className="text-lg text-gray-600 dark:text-gray-300 mb-8">
          The page you are looking for might have been moved, deleted, or never existed.
        </p>
        <Link
          href="/"
          className="px-6 py-3 bg-film-red-600 text-white font-medium rounded-lg hover:bg-film-red-700 transition-colors inline-block"
        >
          Back to Home
        </Link>
      </div>
...
const NotFoundPage = () => {
  return (
    <Suspense fallback={<SimpleNotFoundLoading />}>
      <SimpleNotFoundContent />
    </Suspense>
  );
};
